...
import {
  ImportResult,
  ImportSummary,
} from "../../../../domain/value-objects/ImportResult.vo";
...
export class ImportSinhVienUseCase {
  ...
  async execute(
    ...
    try {
      // Step 1: Parse input via strategy (Excel/CSV tuỳ strategy)
  ...
  private async importSingleRecord(
    ...
    if (existingMssv) {
      results.push(
        ImportResult.failed(rowNumber, "Mã số sinh viên đã tồn tại")
      );
      return;
    }
    ...
    const khoa = await this.unitOfWork
      .getKhoaRepository()
      .findByMaKhoa(record.maKhoa);

    if (!khoa) {
      results.push(
        ImportResult.failed(rowNumber, `Không tìm thấy khoa: ${record.maKhoa}`)
      );
      return;
    }
    ...
    const nganh = await this.unitOfWork
      .getNganhRepository()
      .findByMaNganh(record.maNganh);

    if (!nganh) {
      results.push(
        ImportResult.failed(
          rowNumber,
          `Không tìm thấy ngành: ${record.maNganh}`
        )
      );
      return;
    }
    ...
    await this.unitOfWork.transaction(async (repos) => {
      ...
      // ✅ Chỉ sửa đúng 1 chỗ: email = MSSV đã bỏ dấu chấm
      const emailLocal = (record.maSoSinhVien || "").replace(/\./g, "");
      const userId = await repos.usersRepo.create({
        id: taiKhoanId,
        hoTen: record.hoTen,
        taiKhoanId,
        maNhanVien: record.maSoSinhVien,
        email: `${emailLocal}@student.hcmue.edu.vn`,
      });
